...
class DelphiMigratorApp(ctk.CTk):
    def __init__(self):
        ...
        self.container_frame.grid(row=0, column=1, sticky="nsew", padx=40, pady=40)
    ...
    def _create_sidebar(self):
        ...
        # Brand Logo
        self.logo_label = ctk.CTkLabel(self.sidebar_frame, text=self._("logo_title"), font=ctk.CTkFont(family="Inter", size=22, weight="bold"), text_color=COLOR_PRIMARY)
        self.logo_label.grid(row=0, column=0, padx=20, pady=(30, 40), sticky="w")

        # Fake Navigation Menu for aesthetic
        self.lbl_menu = ctk.CTkLabel(self.sidebar_frame, text=self._("lbl_menu"), font=ctk.CTkFont(family="Inter", size=11, weight="bold"), text_color=COLOR_SECONDARY)
        self.lbl_menu.grid(row=1, column=0, padx=20, pady=(0, 10), sticky="w")

        self.btn_nav1 = ctk.CTkButton(self.sidebar_frame, text=self._("nav_dashboard"), font=ctk.CTkFont(size=14), fg_color=BG_INPUT, text_color=COLOR_PRIMARY, anchor="w", height=40, command=self.show_dashboard)
        self.btn_nav1.grid(row=2, column=0, padx=15, pady=5, sticky="ew")

        self.btn_nav2 = ctk.CTkButton(self.sidebar_frame, text=self._("nav_settings"), font=ctk.CTkFont(size=14), fg_color="transparent", text_color=COLOR_SECONDARY, anchor="w", height=40, hover_color=BG_INPUT, command=self.show_settings)
        self.btn_nav2.grid(row=3, column=0, padx=15, pady=5, sticky="ew")

        # Bottom Mini-Player / Profile mimic
        self.profile_frame = ctk.CTkFrame(self.sidebar_frame, fg_color=BG_INPUT, corner_radius=12)
        self.profile_frame.grid(row=5, column=0, padx=15, pady=20, sticky="ew")
        
        self.lbl_user_title = ctk.CTkLabel(self.profile_frame, text=self._("user_title"), font=ctk.CTkFont(size=13, weight="bold"), text_color=COLOR_PRIMARY)
        self.lbl_user_title.pack(padx=10, pady=(10, 0), anchor="w")
        self.lbl_user_sub = ctk.CTkLabel(self.profile_frame, text=self._("user_sub"), font=ctk.CTkFont(size=11), text_color=COLOR_SECONDARY)
        self.lbl_user_sub.pack(padx=10, pady=(0, 10), anchor="w")
    ...
    def _create_dashboard_frame(self):
        ...
        self.header_frame.grid(row=0, column=0, columnspan=2, sticky="ew", pady=(0, 30))
        self.header_frame.grid_columnconfigure(0, weight=1)

        self.header_label = ctk.CTkLabel(self.header_frame, text=self._("header_title"), font=ctk.CTkFont(family="Inter", size=32, weight="bold"), text_color=COLOR_PRIMARY)
        self.header_label.grid(row=0, column=0, sticky="w")

        # Horizontal Cards Row
        # Card 1: Source
        self.card_source = ctk.CTkFrame(self.dashboard_frame, fg_color=CARD_1, corner_radius=24, height=180)
        self.card_source.grid(row=1, column=0, sticky="nsew", padx=(0, 10))
        self.card_source.grid_propagate(False)

        self.lbl_src_title = ctk.CTkLabel(self.card_source, text=self._("card_src_title"), font=ctk.CTkFont(size=20, weight="bold"), text_color="#FFFFFF")
        self.lbl_src_title.pack(anchor="w", padx=25, pady=(25, 5))
        self.lbl_src_sub = ctk.CTkLabel(self.card_source, text=self._("card_src_sub"), font=ctk.CTkFont(size=13), text_color="#E0E0FF")
        self.lbl_src_sub.pack(anchor="w", padx=25, pady=(0, 20))

        ent_source = ctk.CTkEntry(self.card_source, textvariable=self.source_dir, fg_color="#4343D9", border_width=0, text_color="#FFFFFF", height=35)
        ent_source.pack(fill="x", padx=25, pady=(0, 10))

        self.btn_source = ctk.CTkButton(self.card_source, text=self._("btn_browse"), command=self.browse_source, fg_color="#FFFFFF", text_color=CARD_1, hover_color="#E0E0FF", font=ctk.CTkFont(weight="bold"), corner_radius=12)
        self.btn_source.pack(anchor="w", padx=25)

        # Card 2: Destination
        self.card_dest = ctk.CTkFrame(self.dashboard_frame, fg_color=CARD_2, corner_radius=24, height=180)
        self.card_dest.grid(row=1, column=1, sticky="nsew", padx=(10, 0))
        self.card_dest.grid_propagate(False)

        self.lbl_dst_title = ctk.CTkLabel(self.card_dest, text=self._("card_dst_title"), font=ctk.CTkFont(size=20, weight="bold"), text_color="#FFFFFF")
        self.lbl_dst_title.pack(anchor="w", padx=25, pady=(25, 5))
        self.lbl_dst_sub = ctk.CTkLabel(self.card_dest, text=self._("card_dst_sub"), font=ctk.CTkFont(size=13), text_color="#FFE0D0")
        self.lbl_dst_sub.pack(anchor="w", padx=25, pady=(0, 20))

        ent_dest = ctk.CTkEntry(self.card_dest, textvariable=self.dest_dir, fg_color="#D97A53", border_width=0, text_color="#FFFFFF", height=35)
        ent_dest.pack(fill="x", padx=25, pady=(0, 10))

        self.btn_dest = ctk.CTkButton(self.card_dest, text=self._("btn_browse"), command=self.browse_dest, fg_color="#FFFFFF", text_color=CARD_2, hover_color="#FFE0D0", font=ctk.CTkFont(weight="bold"), corner_radius=12)
        self.btn_dest.pack(anchor="w", padx=25)

        # Options Section
        self.lbl_options = ctk.CTkLabel(self.dashboard_frame, text=self._("lbl_options"), font=ctk.CTkFont(family="Inter", size=18, weight="bold"), text_color=COLOR_PRIMARY)
        self.lbl_options.grid(row=2, column=0, columnspan=2, sticky="w", pady=(40, 15))

        self.options_frame = ctk.CTkFrame(self.dashboard_frame, fg_color="transparent")
        self.options_frame.grid(row=3, column=0, sticky="nsew")
        ...
        self.chk_utf8.pack(anchor="w", pady=8)
        self.chk_bde = ctk.CTkCheckBox(self.options_frame, text=self._("chk_bde"), variable=self.var_bde, text_color=COLOR_SECONDARY, fg_color=CARD_1, font=chk_font)
        ...
        self.right_col_frame.grid(row=3, column=1, sticky="nsew", padx=(20, 0))

        self.btn_start = ctk.CTkButton(self.right_col_frame, text=self._("btn_start_ready"), command=self.start_migration, font=ctk.CTkFont(size=14, weight="bold"), fg_color=COLOR_PRIMARY, text_color=BG_MAIN, hover_color="#E5E5E5", corner_radius=24, height=45)
        self.btn_start.pack(fill="x", pady=(0, 15))

        self.log_textbox = ctk.CTkTextbox(self.right_col_frame, fg_color=BG_INPUT, text_color=COLOR_SECONDARY, corner_radius=16, font=ctk.CTkFont(family="Consolas", size=11))
